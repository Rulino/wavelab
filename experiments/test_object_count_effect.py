...
#
# Testy pre 2, 4 a 6 objects.
# Creates three scenes, runs the solver, and saves results to:
...
MATERIALS = {
    "air": {"c": 3.0e8, "absorption": 0.03, "T": 1.00, "R": 0.00, "scatter": 0.00},
    "metal": {"c": 2.5e8, "absorption": 0.10, "T": 0.03, "R": 0.90, "scatter": 1.00, "barrier": True},
    "concrete": {"c": 2.0e8, "absorption": 0.08, "T": 0.18, "R": 0.60, "scatter": 0.55, "barrier": True},
    "glass": {"c": 2.2e8, "absorption": 0.05, "T": 0.65, "R": 0.15, "scatter": 0.25},
    "plastic": {"c": 2.4e8, "absorption": 0.05, "T": 0.82, "R": 0.10, "scatter": 0.18},
    "water": {"c": 1.5e8, "absorption": 0.06, "T": 0.85, "R": 0.10, "scatter": 0.25},
}

ALL_OBJECTS = [
    {"x": 0.75, "y": 0.55, "material": "metal", "shape": "triangle", "r": 0.10, "width": 0.20, "height": 0.20, "angle": 0},
    {"x": 1.20, "y": 1.25, "material": "concrete", "shape": "rectangle", "r": 0.09, "width": 0.18, "height": 0.18, "angle": 0},
    {"x": 1.75, "y": 0.85, "material": "glass", "shape": "circle", "r": 0.10, "width": 0.20, "height": 0.20, "angle": 0},
    {"x": 2.30, "y": 1.55, "material": "plastic", "shape": "triangle", "r": 0.09, "width": 0.18, "height": 0.20, "angle": 0},
    {"x": 2.65, "y": 0.35, "material": "water", "shape": "circle", "r": 0.10, "width": 0.20, "height": 0.20, "angle": 0},
    {"x": 0.45, "y": 1.55, "material": "concrete", "shape": "square", "r": 0.08, "width": 0.16, "height": 0.16, "angle": 25},
]
...
def main():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Test objects: 2 / 4 / 6\n")
        f.write("=" * 42 + "\n\n")
        for r in results:
            f.write(f"Objects: {r['objects']}\n")
            f.write(f"Accuracy: {r['accuracy_percent']:.6f}%\n")
            f.write(f"Mean absolute error: {r['mean_abs_error']:.8f}\n")
            f.write(f"Maximum absolute error: {r['max_abs_error']:.8f}\n")
    ...
    for r in results:
        print(f"{r['objects']} objects -> {r['folder']}")
