...
def run(cfg: ModulusConfig) -> None:
    ...
    absorption_air = 0.1
    # Second medium: water (moderate absorption)
    absorption_water = 0.5
    amplitude = 1.0
    lambda_val = 0.3
    # Hranica between media – e.g. x < 1.5: air, x >= 1.5: water
    boundary_x = 1.5

    # Coefficients for reflection and transmission – at the air-water (water transmits most waves)
    R = 0.1   # low reflection
    T = 0.9   # high transmission 
    ...
    # Transmission term: vlna, transmitted into water (with higher T and attenuation in water)
    u_transmitted = amplitude * T * sin(2*pi*total_path/lambda_val) * exp(-absorption_water*total_path)
    
    # Scattering term: additional contribution, generated at the interface, computed from r_out.
    # The scattering amplitude is computed z amplitude a absorption_water:
    ...
    # The resulting wave in water is sum of transmission and scattering
    u_expr_water = u_transmitted + u_scattered

    # -------------------------
    # Combination cez Piecewise:
    # For x < boundary_x use air region, for x >= boundary_x water medium.
    # -------------------------
    u_expr = Piecewise(
